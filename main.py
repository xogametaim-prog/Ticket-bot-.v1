...
async def احصل_على_سلعة_من_المتجر(رقم_السلعة):
    async with aiosqlite.connect(مسار_قاعدة_البيانات) as قاعدة:
        async with قاعدة.execute("SELECT item_id, الاسم, سعر_عملات, سعر_رصيد, الوصف FROM المتجر WHERE item_id = ?", (رقم_السلعة,)) as مؤشر:
            الصف = await مؤشر.fetchone()
            return {"id": الصف[0], "name": ... if الصف is None else الصف[1], "coinPrice": ... if الصف is None else الصف[2], "creditPrice": ... if الصف is None else الصف[3], "desc": ... if الصف is None else الصف[4]} if الصف else None
...
async def احصل_على_سلعة_من_السوق_السوداء(رقم_السلعة):
    async with aiosqlite.connect(مسار_قاعدة_البيانات) as قاعدة:
        async with قاعدة.execute("SELECT item_id, الاسم, سعر_عملات, سعر_رصيد, الوصف FROM السوق_السوداء WHERE item_id = ?", (رقم_السلعة,)) as مؤشر:
            الصف = await مؤشر.fetchone()
            return {"id":  ... if الصف is None else الصف[0], "name":  ... if الصف is None else  الصف[1], "coinPrice": ... if الصف is None else  الصف[2], "creditPrice": ... if الصف is None else  الصف[3], "desc":  ... if الصف is None else الصف[4]} if الصف else None
...
async def تفعيل_فريق_امر(التفاعل: discord.Interaction, الرقم: int):
    المعرف = str(التفاعل.user.id)
...
# ----- أمر الشراء والعلاج الفوري الذكي والمنظم -----
...
async def اشتري(التفاعل: discord.Interaction, رقم_السلعة: int, العملة: str, الكمية: int = 1):
    ...
    # فحص الرصيد أولاً قبل أي شيء لضمان الأمان ومكافحة الثغرات
    if العملة == "coins":
    ...
    # ========== نظام العلاج الفوري (للتفاحة رقم 1 والجرعة رقم 5) ==========
    if رقم_السلعة in [1, 5]:
        ...
        # منع الشراء إذا كانت الصحة ممتلئة بالفعل
        if صحة_الحالية >= 200:
        ...
        # الخصم المالي بعد التأكد من الاحتياج الفعلي والقدرة المالية
        if العملة == "coins":
    ...
    # ========== السلع العادية (يتم إضافتها إلى المخزون تلقائياً) ==========
    if العملة == "coins":
        await تحديث_مستخدم(المعرف, عملات=المستخدم["عملات"] - التكلفة)
    else:
        await تحديث_مستخدم(المعرف, رصيد=المستخدم["رصيد"] - التكلفة)
...
if __name__ == "__main__":
    # تشغيل Flask في خيط مستقل لـ Render
