...
client.on('messageCreate', async message => {
    ...
    // إعداد الترحيب بالأعضاء -wel
    if (message.content.startsWith(WELCOME_SETUP_PREFIX)) {
        if (!message.member.permissions.has(PermissionFlagsBits.Administrator)) return;
        const channelMention = message.mentions.channels.first();
        if (!channelMention) return message.reply('❌ يرجى منشن روم الترحيب المخصص (مثال: `-wel #chat`):');
        
        tempSetup.set('welcome_channel_id', channelMention.id);
        await message.reply(`✅ **تم تعيين روم ترحيب الأعضاء على: ${channelMention}**`);
        await message.delete().catch(() => {});
        return;
    }
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isButton()) {
        ...
        if (customId.startsWith('close_custom_ticket_')) {
            ...
            // ميزة إرسال التقييم بالخاص بشكل فوري دون تأخير (حل مشكلة التقييم)
